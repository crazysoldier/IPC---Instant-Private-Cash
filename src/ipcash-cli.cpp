...
std::string HelpMessageCli()
{
    ...
    strUsage += "  -rpcport=<port>        " + strprintf(_("Connect to JSON-RPC on <port> (default: %u or testnet: %u)"), 55993, 45993) + "\n";
...
static bool AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc < 2 || mapArgs.count("-?") || mapArgs.count("-help") || mapArgs.count("-version")) {
        std::string strUsage = _("Ipcash Core RPC client version") + " " + FormatFullVersion() + "\n";
        if (!mapArgs.count("-version")) {
            strUsage += "\n" + _("Usage:") + "\n" +
                        "  ipcash-cli [options] <command> [params]  " + _("Send command to Ipcash Core") + "\n" +
                        "  ipcash-cli [options] help                " + _("List commands") + "\n" +
                        "  ipcash-cli [options] help <command>      " + _("Get help for a command") + "\n";
