...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeIpcashAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeIpcashAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeIpcashAmount = strAnonymizeIpcashAmount.remove(strAnonymizeIpcashAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeIpcashAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeIpcashAmount * COIN) nMaxToAnonymize = nAnonymizeIpcashAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeIpcashAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeIpcashAmount));
        strAnonymizeIpcashAmount = strAnonymizeIpcashAmount.remove(strAnonymizeIpcashAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeIpcashAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
