...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a IPC address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no IPC: URI
...
{
    // Convert ipcash:// to ipcash:
    //
    //    Cannot handle this later, because ipcash:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open ipcash.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "IPC.lnk";
}
...
{
    return GetAutostartDir() / "ipcash.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a ipcash.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the ipcash app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add ipcash app to startup item list
